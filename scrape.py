...
def scrape():
    ...
    # Path for Chromdriver to execute
    ...
    # Retrieve the parent item for all news items on the website using beauitful soup
    results = soup.find_all('li', class_='slide')
    #print("Results: ", results[0])

    # Find the header and paragraph text from the first news items on the website
    news_paragraph = results[0].find('div', class_='article_teaser_body').text
    news_header = results[0].find('h3').text

    # Add news title and news paragraph to our list
    mars_data_list.append({'title' : 'Latest Mars News', 'header' : news_header, 'paragraph': news_paragraph, 'url' : mars_news_url})

    # Close and exit browser session
    browser.quit()


    # URL for feature image of Mars
